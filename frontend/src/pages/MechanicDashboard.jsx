...
function AppointmentCard({ appointment, onStatusChange, updating }) {
  const { client, car, repairDetails, total_price, status, date, notes } = appointment;

  const nextStatus = {
    pending:     { value: 'in_progress', label: '▶ Розпочати роботу',  cls: 'btn-primary w-full justify-center' },
    in_progress: { value: 'done',        label: '✓ Позначити готовим', cls: 'w-full justify-center inline-flex items-center gap-2 px-4 py-2.5 bg-green-700 hover:bg-green-600 text-white font-medium text-sm rounded-lg transition-all active:scale-95' },
    done:        null,
  };
  ...
  return (
    <div className={`card border-dark-700 space-y-4 animate-fadeIn ${
      ...
      {car?.make ? (
        <div className="flex items-center gap-3 p-3 bg-brand-500/10 border border-brand-500/25 rounded-xl">
          ...
          <div className="min-w-0">
            ...
            {car.vin && (
              <p className="text-brand-400/60 text-xs font-mono mt-0.5">{car.vin}</p>
            )}
          </div>
          <span className="ml-auto text-xs text-brand-400/70 flex-shrink-0">авто у наряді</span>
        </div>
      ) : client?.cars?.length > 0 ? (
        <div className="space-y-1.5">
          <p className="text-xs text-dark-500 uppercase tracking-wider">Авто клієнта</p>
          <div className="flex flex-wrap gap-2">
            {client.cars.map((c, i) => (
              <div key={i} className="flex items-center gap-2 px-3 py-1.5 bg-dark-900 border border-dark-700 rounded-lg">
                ...
                <div>
                  <p className="text-dark-300 text-xs font-medium">{c.make}</p>
                  {c.vin && <p className="text-dark-600 text-xs font-mono">{c.vin}</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      {repairDetails?.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-xs text-dark-400 uppercase tracking-wider font-medium">Роботи та запчастини</p>
          {repairDetails.map((rd, i) => (
            <div key={i} className="flex items-center justify-between gap-3 py-2 border-b border-dark-700/50 last:border-0">
              <div className="flex items-center gap-2 min-w-0">
                ...
                {rd.quantity > 1 && <span className="text-dark-500 text-xs flex-shrink-0">×{rd.quantity}</span>}
              </div>
      ...
      {next && (
        ...
        >
          {updating === appointment._id ? (
            <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Оновлення...</>
          ) : next.label}
        </button>
      )}
        ...
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={fetchAppointments} disabled={loading}
              className="p-2 text-dark-400 hover:text-dark-100 hover:bg-dark-800 rounded-lg transition-all" title="Оновити">
      ...
      <main className="max-w-2xl mx-auto px-4 py-5 space-y-5">
        ...
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-brand-500/20 border border-brand-500/30 rounded-xl flex items-center justify-center flex-shrink-0">
            <span className="text-brand-400 font-display font-bold text-lg">{user?.fullName?.charAt(0) || 'М'}</span>
          </div>
          <div>
        ...
        <div className="flex gap-2 bg-dark-900 border border-dark-700 p-1 rounded-xl">
          {[{ value: 'active', label: '🔥 Активні' }, { value: 'all', label: 'Всі наряди' }].map(f => (
            <button key={f.value} onClick={() => setFilterStatus(f.value)}
              className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${
                filterStatus === f.value ? 'bg-brand-500 text-white shadow-sm' : 'text-dark-400 hover:text-dark-200'
              }`}>{f.label}</button>
          ))}
        </div>
